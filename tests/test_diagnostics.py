...
class TestAssertNoTemporalLeakage:
    ...
    def test_purge_horizon_extends_zone(self) -> None:
        ...
        test_idx = np.arange(10, 15)
        # Without padding, train row 8 [Jan 9, Jan 10] ends before test start
        # Jan 11, so there is no overlap.
        ...
        # and train row 8 [Jan 9, Jan 10] touches (so overlaps) it.
    ...
    def test_disjoint_test_blocks_check_local_horizons(self) -> None:
        ...
        Rows 3 and 8 touch a test block (closed intervals) and are excluded."""
        pred, evalu = _make_horizon_dataset(horizon_days=1, n=12)
        train_idx = np.array([4, 5, 6, 7])
...
class TestComputeOverlapFraction:
    ...
    def test_disjoint_test_blocks_do_not_count_middle_rows_as_overlap(self) -> None:
        ...
        test_idx = np.array([0, 1, 2, 9, 10, 11])
        # Only rows 3 and 8 touch a test block; the middle rows 4-7 do not count.
        assert compute_overlap_fraction(train_idx, test_idx, pred, evalu) == pytest.approx(2 / 6)
