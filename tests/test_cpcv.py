...
class TestCombinatorialPurgedCV:
    def test_observation_embargo_applies_after_each_non_adjacent_test_block(self) -> None:
        ...
        # Labels of block {0..3} end at row 4 and of block {8..11} at row 12
        # (AFML 7.2/7.3 anchor), so 1 embargo row after each is 5 and 13.
        # Row 7 [Jan 8, Jan 9] touches test row 8 and is purged.
        np.testing.assert_array_equal(train_idx, np.array([6, 14, 15]))
