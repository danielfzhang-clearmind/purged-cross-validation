...
def test_user_story_clinical_split_clean() -> None:
    ...
    first 69 days train, last 30 days test, with a 24h prediction horizon.
    Day 70's label [Mar 10, Mar 11] would end exactly when the test starts,
    so it is left out (closed intervals, AFML 7.4.1)."""
    pred = pd.Series(pd.date_range("2024-01-01", periods=100, freq="D"))
    evalu = pred + pd.Timedelta(days=1)
    train_idx = np.arange(0, 69)
    test_idx = np.arange(70, 100)
    assert_no_temporal_leakage(train_idx, test_idx, pred, evalu)
    with pytest.raises(TemporalLeakageError, match="row 69"):
        assert_no_temporal_leakage(np.arange(0, 70), test_idx, pred, evalu)
...
def test_user_story_overlapping_labels_caught() -> None:
    ...
    leaking training row in order. Row 13 has horizon [Jan 14, Jan 21] which
    touches (so overlaps) [Jan 21, Feb 6], so the error names row 13.
    """
    ...
    with pytest.raises(TemporalLeakageError) as exc_info:
        assert_no_temporal_leakage(train_idx, test_idx, pred, evalu)
    assert "row 13" in str(exc_info.value)
