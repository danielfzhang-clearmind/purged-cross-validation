...
@pytest.mark.e2e
def test_user_story_touching_intervals_overlap() -> None:
    """Closed convention (AFML 7.4.1): a 1-hour horizon ending at 09:00 and
    one starting at 09:00 both depend on the 09:00 observation, so they
    overlap. One starting at 09:01 does not."""
    a_start = pd.Timestamp("2024-03-15 08:00")
    a_end = pd.Timestamp("2024-03-15 09:00")
    b_start = pd.Timestamp("2024-03-15 09:00")
    b_end = pd.Timestamp("2024-03-15 10:00")
    assert horizons_overlap(a_start, a_end, b_start, b_end) is True
    assert horizons_overlap(a_start, a_end, pd.Timestamp("2024-03-15 09:01"), b_end) is False
