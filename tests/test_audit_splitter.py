...
def test_audit_reports_real_purge_and_embargo_stages() -> None:
    ...
    first = report.iloc[0]
    # Padded test horizon is [Dec 31, Jan 7] (closed): rows 5 [Jan 6, Jan 7] and
    # 6 [Jan 7, Jan 8] are purged. Test labels end at row 5, so the 2-row
    # embargo covers rows 6-7; only row 7 is left for it to remove.
    assert first["candidate_train_size"] == 15
    assert first["rows_removed_by_purge"] == 2
    ...
    assert first["final_train_size"] == 12
    assert bool(first["train_nonempty"])
    assert first["candidate_overlap_fraction"] == pytest.approx(2 / 15)
    ...
    assert first["train_time_envelope_start"] == pd.Timestamp("2024-01-09")
...
def test_sliding_window_removals_are_reported_as_finalization() -> None:
    ...
    # With purge_horizon=1D the two rows before each test block overlap or
    # touch its padded closed horizon.
    assert report["rows_removed_by_purge"].tolist() == [2, 2, 2]
    assert report["rows_removed_by_embargo"].tolist() == [0, 0, 0]
    assert report["rows_removed_by_finalization"].tolist() == [7, 9, 11]
...
def test_finalizer_replacement_reports_one_removal_and_one_addition() -> None:
    ...
    # One of the 5 candidates touches the test row and is purged first.
    assert report.loc[0, "final_train_size"] == 4
