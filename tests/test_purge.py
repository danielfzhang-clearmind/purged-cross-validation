...
class TestPurgeBasic:
    ...
    def test_disjoint_test_blocks_do_not_purge_middle_train_block(self) -> None:
        ...
        convex-hull purge window. Only rows 3 [Jan 4, Jan 5] and 8 [Jan 9, Jan 10],
        which touch a test block, are purged (closed intervals, AFML 7.4.1)."""
        ...
        np.testing.assert_array_equal(result, np.array([4, 5, 6, 7]))


class TestPurgeWithPadding:
    def test_padding_extends_purge_zone(self) -> None:
        """1-day horizon, train row 8 [Jan 9, Jan 10] ends before test start Jan 11.
        Without padding -> kept. With purge_horizon=1D the test_start shifts to
        Jan 10, so train_eval(Jan 10) >= Jan 10 -> dropped."""
        pred, evalu = _make_horizon_dataset(horizon_days=1)
        train_idx = np.array([8])
        test_idx = np.arange(10, 15)
...
class TestPurgeAFMLSnippet71:
    ...
    *Advances in Financial Machine Learning* (Lopez de Prado, Wiley 2018),
    ...
    [Jan 6, Jan 11]. Snippet 7.1 compares with ``<=``, so every training row
    whose closed 3-day horizon intersects the test horizon, including rows
    that only touch it, must be dropped.
    """
    ...
    def test_snippet_7_1_purges_expected_rows(self) -> None:
        ...
        # Hand-verified against Snippet 7.1's closed formulation:
        ...
        #  2  | Jan 3  | Jan 6  | False        | False         | no  (touches test start)
    ...
    def test_snippet_7_1_with_purge_horizon_drops_boundary_too(self) -> None:
        ...
        Row 1 (evalu=Jan 5) now touches it, so it is dropped too."""
        pred = pd.Series(pd.date_range("2024-01-01", periods=10, freq="D"))
        evalu = pred + pd.Timedelta(days=3)
        train_idx = np.array([0, 1, 2, 3, 4, 8, 9])
        test_idx = np.array([5, 6, 7])
