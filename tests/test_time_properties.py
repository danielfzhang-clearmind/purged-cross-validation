...
class TestHorizonsOverlapProperties:
    ...
    def test_touching_endpoints_overlap(
        ...
        """Closed invariant (AFML 7.4.1): [a, b] and [b, c] always overlap at b,
        while [a, b] and any interval starting strictly after b never do."""
        b = anchor + pd.Timedelta(seconds=seconds)
        c = b + pd.Timedelta(seconds=seconds)
        assert horizons_overlap(anchor, b, b, c) is True
        assert horizons_overlap(anchor, b, b + pd.Timedelta(1, "ns"), c) is False
