...
class TestBaseTemporalSplitterSplit:
    ...
    def test_split_works_when_no_purge_or_embargo_needed(self) -> None:
        """With zero purge_horizon and embargo and non-overlapping labels,
        train and test are simply complement sets — no rows dropped."""
        pred, evalu = _times(n=20, horizon_days=0)
        cv = _TwoFoldStub(prediction_times=pred, evaluation_times=evalu)
        X = np.zeros((20, 1))  # noqa: N806
        folds = list(cv.split(X))
        # _TwoFoldStub: fold 0 test=[0..9], fold 1 test=[10..19]. Zero-length
        # labels never overlap, so all complementary rows are kept.
    ...
    def test_split_applies_observation_embargo(self) -> None:
        ...
        # Test labels end at row 10; the 3 embargo rows are 11-13.
        np.testing.assert_array_equal(test0, np.arange(10))
        np.testing.assert_array_equal(train0, np.arange(14, 20))
