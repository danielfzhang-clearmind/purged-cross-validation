...
from purgedcv._intervals import overlaps_any_closed_interval
...
def purge(
    ...
    """Drop training rows whose closed label horizon overlaps any test horizon.

    Each test row contributes a closed horizon
    ``[prediction_time - purge_horizon, evaluation_time + purge_horizon]``.
    Horizons are closed as in AFML section 7.4.1: a training label that ends
    exactly when a test label starts, or starts exactly when one ends, shares
    that observation and is purged.
    ...
    A training row at position ``i`` is kept iff its label horizon
    ``[prediction_time, evaluation_time]`` overlaps none of those merged test
    horizons.
    ...
    keep = ~overlaps_any_closed_interval(train_pred, train_eval, test_starts, test_ends)
