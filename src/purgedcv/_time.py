...
def horizons_overlap(
    ...
    """Return ``True`` iff closed intervals ``[a_start, a_end]`` and
    ``[b_start, b_end]`` overlap.

    Touching intervals (``a_end == b_start``) DO overlap: as in AFML section
    7.4.1, labels sharing an endpoint depend on a common observation. The
