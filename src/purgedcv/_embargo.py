...
def _positional_embargo_mask(
    ...
    As in AFML Snippets 7.2/7.3, the window for each contiguous test block is
    counted from the end of that block's labels, not from its last row: the
    anchor is the last position whose ``prediction_time`` is at or before the
    block's latest ``evaluation_time``. Rows after the block up to and including
    ``anchor + embargo_observations`` are embargoed; rows up to the anchor
    overlap the test labels anyway, so this keeps standalone use consistent
    with purge-then-embargo.
    """
    ...
    breaks = np.flatnonzero(np.diff(sorted_test) > 1) + 1
    in_embargo = np.zeros(len(train_idx), dtype=bool)
    for run in np.split(sorted_test, breaks):
        label_end = evaluation_times[run].max()
        anchor = max(int(run[-1]), int(np.flatnonzero(prediction_times <= label_end).max()))
        in_embargo |= (train_idx > run[-1]) & (train_idx <= anchor + embargo_observations)
    return in_embargo
...
def apply_embargo(
    ...
    *,
    ...
    - ``embargo_observations`` drops that many row positions immediately
      after the end of each contiguous test block's labels, i.e. after the
      last row whose ``prediction_time`` is at or before the block's latest
      ``evaluation_time`` (AFML Snippets 7.2/7.3);
    - ``embargo_fraction`` drops ``floor(n_samples * fraction)`` row positions
      after the end of each contiguous test block's labels.
    ...
    Args:
        ...
        embargo_observations: number of row positions to drop after the end
            of each contiguous test block's labels.
        embargo_fraction: fraction of the full dataset to drop after the end
            of each contiguous test block's labels. Must be between 0 and 1 inclusive; the
    ...
    if observations is not None:
        in_embargo = _positional_embargo_mask(
            train_idx, test_idx, prediction_times, evaluation_times, observations
        )
